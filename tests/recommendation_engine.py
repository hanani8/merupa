...
def test_re_invalid_course_no():
    endpoint = base_url + "/api/recommendations?roll_no=&pref_no=invalid&pref_co="
    response = requests.get(endpoint)
    data = response.json()
    # assert response.status_code == 200
    assert data['error'] == True
    assert data['msg'] == "Number of preferred courses expects integer input"
    assert data['data'] == ""
...
def test_re_correct_roll_no_course_no_3():
    ...
    assert data['data']["courses"][0] == "CS2001"
...
def test_re_correct_roll_no():
    ...
    assert data['error'] == False
    assert data['msg'] == "Recommended courses returned"
    assert data['data']["courses"][0] == "MA1004"