...
app = init_app()
from app.course.models import CourseRating
...
def test_course_unsuccessful_edit_feedback():
    ...
    response = requests.put(endpoint, json={"rating_type":"Course Content", "rating_value":7.5, "student_id": 3})
    data = response.json()
    print(data)
    assert response.status_code == 400
...
def test_course_unsuccessful_edit_feedback_wrong_rating_type():
    endpoint = base_url + "/api/course/CS1001/rating"
    response = requests.put(endpoint, json={"rating_type":"Custom Rating", "rating_value":7.5, "student_id": 3})
    data = response.json()
    assert response.status_code == 400
    assert data["error_code"] == "R001"
    assert data["error_message"] == "Rating Not Found"
...
def test_course_successful_edit_feedback():
    endpoint = base_url + "/api/course/MA1003/rating"
    response = requests.put(endpoint, json={"rating_type":"Course Support", "rating_value":8.5, "student_id": 2})
    data = response.json()
    print(data)
    assert response.status_code == 200
    assert data["course_id"] == "MA1003"
