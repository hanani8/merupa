...
def test_course_incorrect_id():
    endpoint = base_url + "/api/course/course/ABC1001"
    response = requests.get(endpoint)
    data = response.json()
    assert response.status_code == 200
...
def test_course_rating_incorrect_course_id():
    ...
    response = requests.post(endpoint, json={"rating_type":"Course Content", "rating_value":7.5, "student_id": 3})
    data = response.json()
    assert response.status_code == 400
    assert data["error_code"] == "C001"
    assert data["error_message"] == "Course Not Found"
...
def test_course_rating_incorrect_student_id():
    endpoint = base_url + "/api/course/MA1001/rating"
    response = requests.post(endpoint, json={"rating_type":"Course Content", "rating_value":7.5, "student_id": 300})
    data = response.json()
    assert response.status_code == 400
